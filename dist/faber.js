...
MediumEditorComponent = (function() {
  ...
  MediumEditorComponent.prototype.template = '/js/components/medium-editor.html';
...
Element1 = (function() {
  ...
  Element1.prototype.template = '/js/components/samples/element1.html';
...
window.faber = angular.module('faber', []).config(function($sceProvider) {
...
faber.controller('BlockController', function($rootScope, $scope, $log, componentsService) {
  ...
  $scope.validateBlock = function(block) {
    ...
    } else if (angular.isObject(block.component) && angular.isString(block.component.template)) {
  ...
  $scope.add = function(block) {
    ...
    if ($scope.validateBlock(block)) {
      ...
      $log.warn({
        'cannot find a component of the given template': block.component
      });
      return false;
    }
  };
  ...
  return $scope.$watch('block.component', function(val) {
    ...
    if (!component && componentsService.getAll().length > 0) {
      return $log.warn({
        'cannot find a component of the given template': val
      });
    } else {
...
faber.controller('EditorController', function($rootScope, $scope, $controller, $log, contentService, componentsService, faberConfig) {
  var validateElementBlock, validateGroupBlock, validateImported;
...
faber.directive('faberBlock', function($compile) {
  return {
    scope: {
    ...
    link: function($scope, $element, attrs) {
      ...
      $scope.$watch('component', function() {
        ...
        if ($scope.component && $scope.component.template) {
          ...
          return $http.get("" + componentTemplateUrl, {
            cache: $templateCache
...
faber.directive('faberEditor', function($rootScope, $rootElement) {
  return {
    restrict: 'AE',
    templateUrl: 'faber-editor.html',
    controller: 'EditorController',
    link: function($scope, $element, attrs) {
      return document.addEventListener('click', function(evt) {
...
angular.module("faber").run(["$templateCache", function($templateCache) {$templateCache.put("faber-block.html","<div ng-class=\"{selected: renderer.isSelected}\" class=\"block\"><div class=\"faber-block-actions\"><button ng-click=\"removeSelf()\" class=\"faber-block-close fa fa-trash-o\"></button></div><faber-component-renderer></faber-component-renderer></div>");
$templateCache.put("faber-components.html","<div ng-click=\"toggleComponents()\"><i ng-if=\"!showingComponents\" class=\"fa fa-plus-square\"></i><ul ng-if=\"showingComponents\" class=\"faber-available-components\"><li ng-repeat=\"comp in components()\" class=\"faber-component\"><button ng-click=\"insertBlock($event, {component: comp.template })\">{{comp.name}}</button></li></ul></div>");
$templateCache.put("faber-editor.html","<faber-components></faber-components><div class=\"faber-blocks\"><div ng-repeat=\"data in block.blocks\" class=\"faber-block-item\"><faber-block data-faber-block-content=\"data\"></faber-block><faber-components></faber-components></div></div>");}]);
faber.factory('componentsService', function($filter, $log) {
  var components, validate;
  components = [];
  validate = function(component) {
    return (angular.isObject(component.inputs) || !component.inputs) && angular.isString(component.template) && (component.type === 'element' || component.type === 'group');
  };
  return {
    init: function(list) {
